...
-- =============================================
CREATE PROCEDURE [dbo].[GetClientSupplierAnalysisAllStatueseGraph]
		@PeriodNumber int,
		@PeriodChoice int,
		@Current bit,
...
if(@index <> 0)    
  set @sql1 =  '(Select InquiryStatus,
         ...
         from (  SELECT ''Inquiry'' InquiryStatus, YEAR(InquiryDate) AS Year,
				 ...
				 where BranchID = ' + @BranchID +' and InquiryDate >= (SELECT DATEADD(dd, 0, DATEDIFF(dd, 0,'''+ Convert(nvarchar(12),@FromDate,100) +'''))) and 
				       InquiryDate <= (SELECT DATEADD(dd, 0, DATEDIFF(dd, 0, '''+ Convert(nvarchar(12),@ToDate,100) +''')))' + @suppliers +' and ' + @ChoosenProductTypes + '
  ...
  set @sql2 = @sql2 +  ' (Select InquiryStatus,
         ...
         from (  SELECT ''Offer'' InquiryStatus, YEAR(OfferDate) AS Year,
				 ...
				 where BranchID = ' + @BranchID +' and OfferDate is not null and 
				       OfferDate >= (SELECT DATEADD(dd, 0, DATEDIFF(dd, 0,'''+ Convert(nvarchar(12),@FromDate,100) +'''))) and 
				       OfferDate <= (SELECT DATEADD(dd, 0, DATEDIFF(dd, 0, '''+ Convert(nvarchar(12),@ToDate,100) +''')))' + @suppliers +' and ' + @ChoosenProductTypes + '
 ...
 if(@index <>0)    
  ...
  set @sql3 = @sql3 +  ' (Select InquiryStatus,
         ...
         from (  SELECT ''Order'' InquiryStatus, YEAR(OrderDate) AS Year,
				 ...
				 FROM         #tempViewSubProjectTotals
				 where BranchID = ' + @BranchID +' and OrderDate is not null and 
				       OrderDate >= (SELECT DATEADD(dd, 0, DATEDIFF(dd, 0,'''+ Convert(nvarchar(12),@FromDate,100) +'''))) and 
				       OrderDate <= (SELECT DATEADD(dd, 0, DATEDIFF(dd, 0, '''+ Convert(nvarchar(12),@ToDate,100) +''')))' + @suppliers +' and ' + @ChoosenProductTypes + '
  ...
  set @sql4 = @sql4 +  ' (Select InquiryStatus,
         ...
         from (  SELECT ''Delivered'' InquiryStatus, YEAR(ShipmentDate) AS Year,
				 ...
				 where BranchID = ' + @BranchID +' and ShipmentDate is not null and 
				       ShipmentDate >= (SELECT DATEADD(dd, 0, DATEDIFF(dd, 0,'''+ Convert(nvarchar(12),@FromDate,100) +'''))) and 
				       ShipmentDate <= (SELECT DATEADD(dd, 0, DATEDIFF(dd, 0, '''+ Convert(nvarchar(12),@ToDate,100) +''')))' + @suppliers +' and ' + @ChoosenProductTypes + '
         ...
         from (  SELECT InquiryStatus, YEAR(LostDate) AS Year,
                 Month(LostDate) AS Month, COUNT(''Lost'') AS StatusCount,
                            CustomerID, SupplierID, SupplierName, CompanyName, 
                             ISNull(Sum(QuotationPriceInEuro),0) AS  TotalAmount
				 FROM         #tempViewSubProjectTotals
				 where BranchID = ' + @BranchID +' and LostDate >= (SELECT DATEADD(dd, 0, DATEDIFF(dd, 0,'''+ Convert(nvarchar(12),@FromDate,100) +'''))) and 
				       LostDate <= (SELECT DATEADD(dd, 0, DATEDIFF(dd, 0, '''+ Convert(nvarchar(12),@ToDate,100) +''')))' + @suppliers +' and ' + @ChoosenProductTypes + '
