...
-- =============================================
CREATE PROCEDURE [dbo].[GetClientAnalysisAllStatusesGraphs]
...
(InquiryStatus nvarchar(50), TotalAmount decimal(18,4), 
Name nvarchar(100), Code nvarchar(15), StatusCount int)
create table #selectedNames (Name nvarchar(100),Code nvarchar(15)) 
 
-- Principle Client Analysis
if @ReportType = 0
insert into  #tempTable execute [dbo].[GetPrincipleClientAnalysisAllStatusesGraph] @PeriodNumber,@PeriodChoice,@Current,@InquiryStatus,
                                 @ID,@SelectedIDs,@IsFromToChecked,@ChoosenFromDate,
                                 @ChoosenToDate,@ChoosenProductTypes,@BranchID
-- Client Supplier Analysis                                 
else if @ReportType = 1
insert into  #tempTable  execute [dbo].[GetClientSupplierAnalysisAllStatueseGraph] @PeriodNumber,@PeriodChoice,@Current,@InquiryStatus,
                                 @ID,@SelectedIDs,@IsFromToChecked,@ChoosenFromDate,
                                 @ChoosenToDate,@ChoosenProductTypes,@BranchID
...
if(@index > 0)    
	...
	#selectedNames.Name as CompanyName, #selectedNames.Code as CompanyCode,	ISNull(t.StatusCount,0) StatusCount 
